...
`WhisperASRPipeline.from_pretrained(weights_dir=..., adapter_dir=...)` against the digest-verified base
snapshot with a weight-level merge check. Like the inference notebook it is standalone (NOTEBOOK_SPEC 2.0):
generated by `tools/build_notebook.py --template tools/notebook_template_finetune.py`, carrying the package
module verbatim, with pins from `tools/finetune-pins.txt`. It needs a 16 GiB-class
