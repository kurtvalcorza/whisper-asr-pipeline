...
def test_par3_generator_check_is_clean(notebook: dict) -> None:
    ...
    assert current == rendered, "notebook is stale; run python tools/build_notebook.py --template tools/notebook_template_finetune.py"
...
def test_finetune_pins_file_carries_pyproject_dependencies_and_extras() -> None:
    """tools/finetune-pins.txt must be pyproject's runtime dependencies plus the tutorial/finetune extras (ENV2)."""
    import tomllib

    project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
    expected = list(project["dependencies"])
    for extra in ("tutorial", "finetune"):
        expected += project["optional-dependencies"][extra]
    assert build._pins(ROOT, TEMPLATE) == expected


def test_finetune_notebook_declares_e2e_and_stays_gpu_honest(notebook: dict) -> None:
    meta = notebook["metadata"]["dimer"]
    assert meta["notebook_profile"] == "E2E" and meta["notebook_mode"] == "GUIDED"
    first = _source(notebook["cells"][0])
    assert "**Run all:**" in first and "CUDA GPU" in first
    # Stage cells only: the carried module's own text is exempt (its error message names the flag).
    stage = "\n".join(
        _source(c)
        for c in _cells(notebook, "code")
        if not c.get("metadata", {}).get("dimer", {}).get("embedded_module")
    )
    assert "load_model(device=DEVICE, weights_dir=WEIGHTS_DIR, allow_download=False)" in stage
